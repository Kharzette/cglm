...
CGLM_INLINE
bool
glm_vec2_eq_eps(const vec2 v, float val) {
...
CGLM_INLINE
void
glm_vec2_sign(const vec2 v, vec2 dest) {
...
CGLM_INLINE
void
glm_vec2_sqrt(const vec2 v, vec2 dest) {
  dest[0] = sqrtf(v[0]);
  dest[1] = sqrtf(v[1]);
}
