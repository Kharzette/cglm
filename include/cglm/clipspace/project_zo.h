...
CGLM_INLINE
void
glm_unprojecti_zo(const vec3 pos, const mat4 invMat, const vec4 vp, vec3 dest) {
  vec4 v;
...
CGLM_INLINE
void
glm_project_zo(const vec3 pos, const mat4 m, const vec4 vp, vec3 dest) {
...
CGLM_INLINE
float
glm_project_z_zo(const vec3 v, const mat4 m) {
  float z, w;
